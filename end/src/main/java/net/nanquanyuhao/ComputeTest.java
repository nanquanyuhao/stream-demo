...
public class ComputeTest {
    ...
    public static void main(String[] args) {
        ...
        Long v1 = list.stream().reduce(Long.valueOf(10), (x1, x2) -> x1 + x2);
        System.out.println(v1);

        // 在串行流(stream)中，该方法跟第二个方法一样，即第三个参数 combiner 不会起作用
        // 即最终结果为求从 0 开始减到最后一个元素的差值，结果：-300
        Long v2 = list.stream().reduce(Long.valueOf(0),
                (x1, x2) -> {
        ...
        Long v3 = list.parallelStream().reduce(Long.valueOf(0),
                (x1, x2) -> {
