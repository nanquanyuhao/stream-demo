...
public class StudentController {
    ...
    // 添加
    @PostMapping("/add")
    public Mono<Student> addHandler(@RequestBody Student student) {
        return client.post()
                .uri("/save")
    ...
    public Mono<Student> modifyHandler(@PathVariable("id") String id,
                                       @RequestBody Student student) {
        return client.put()
                .uri("/updatestat/{id}", id)
    ...
    public Flux<Student> rangeHandler(@PathVariable("below") int below,
